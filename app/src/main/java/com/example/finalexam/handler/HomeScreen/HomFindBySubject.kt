...
class HomFindBySubject(documentDao: DocumentDao,private val listDocument: List<Document>) : IntentHandler<HomeIntent, HomeResult> {
    ...
    override suspend fun handle(intent: HomeIntent, setResult: (HomeResult) -> Unit) {
        ...
        if (intent is HomeIntent.FindTodoBySubject) {
            val keyword = intent.subject // Lấy keyword từ intent

            // Gọi usecase, truyền keyword vào
            val documents = usecase.findDocumentBySubject(keyword,listDocument)
